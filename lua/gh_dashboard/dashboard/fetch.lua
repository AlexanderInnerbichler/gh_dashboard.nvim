...
local function parse_labels(raw)
  local labels = {}
  for _, lbl in ipairs(type(raw) == "table" and raw or {}) do
    if type(lbl) == "string" then
      table.insert(labels, lbl)
    elseif type(lbl) == "table" and lbl.name then
      table.insert(labels, lbl.name)
    end
  end
  return labels
end
...
function M.issues(callback)
  gh.run_with_retry(
    { "gh", "api", "/issues",
      ...
      "--field", "state=open",
      "--field", "per_page=100",
      "--jq", '[.[] | select(.pull_request == null) | {number, title, url: .html_url, created_at, repo: (.repository_url | ltrimstr("https://api.github.com/repos/")), labels: [.labels[].name]}]' },
    function(err, data)
...
function M.activity_feed(callback, own_login)
  ...
  local function collect(err, events)
    ...
    if pending == 0 then
      ...
      local top = {}
      for i = 1, math.min(cap, #all_events) do top[i] = all_events[i] end
